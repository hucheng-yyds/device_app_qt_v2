...
void FaceIdentify::run()
{
    ...
    //                                }
    //                            }
    //                            emit identifySuccess(name, i, iMFaceHandle[i].track_id);
    //                            egPass = true;
                            }
    //                        isStranger = "0";
                        } else {
    //                        emit closeRemark();
                            //emit identifyFailed(tr("未注册"), i, iMFaceHandle[i].track_id, "请刷身份证");
    //                        if(1 == settings->isVi)
    //                        {
    //                            emit identifyFailed(tr("未注册"), i, m_iMFaceHandle[i].track_id, 1, tr("请刷身份证"));
    //                        }
    //                        else if(0 == settings->isTemp){
    //                            emit identifyFailed(tr("未注册"), i, m_iMFaceHandle[i].track_id, 0, tr("请联系管理员"));
    //                        }
    //                        else {
    //                            emit identifyFailed(tr("未注册"), i, m_iMFaceHandle[i].track_id, 0, "");
    //                        }
                            egPass = false;
                            face_id = 0;
                        }
